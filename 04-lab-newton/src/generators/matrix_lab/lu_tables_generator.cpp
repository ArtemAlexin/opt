...
latex_line_data_t run(const data_t &data) {
    ...
    EvalData evalData;
    Vector x = solveLUGauss(ACopy, bCopy);
...
void runRandom() {
    ...
    writeTo("../lu_tables", "random.txt", toLatexTable(tableData));
    cerr << "runRandom done\n";
}

void runSeq() {
    latex_table_data_t tableData;
    vector<size_t> dims = {10, 20, 50};
    for (size_t n : dims) {
    ...
    writeTo("../lu_tables", "seq.txt", toLatexTable(tableData));
    cerr << "runSeq done\n";
}
...
void runInvSeq() {
    ...
    writeTo("../lu_tables", "inv_seq.txt", toLatexTable(tableData));
    cerr << "runInvSeq done\n";
}
...
void runGil() {
    ...
    writeTo("../lu_tables", "gil.txt", toLatexTable(tableData));
    cerr << "runGil done\n";
}
