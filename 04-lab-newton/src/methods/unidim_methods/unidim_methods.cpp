...
namespace UnidimMethods {
    EvalData dichotomyMethod(UndimFunction &f, Segment seg, double eps) {
        ...
        while ((seg.b - seg.a) >= 2 * eps) {
            ...
            data_.addIteration(seg.a, seg.b, {x1, x2}, {f1, f2});
        ...
        return data_.finishEval(seg.a, seg.b, (seg.a + seg.b) / 2);
    }
    ...
    EvalData goldenSectionMethod(UndimFunction &f, Segment seg, double eps) {
        ...
        while (seg.b - seg.a > eps) {
            if (f1 <= f2) {
                ...
                f1 = f(x1);
                data_.addIteration(seg.a, seg.b, {x1}, {f1});
            } else {
                ...
                f2 = f(x2);
                data_.addIteration(seg.a, seg.b, {x2}, {f2});
            }
        }
...
//        data_.add_iteration(seg.a, seg.b, {x1, x2, x3}, {f1, f2, f3});
        ...
        while (x_last == 0 || std::abs(x_curr - x_last) >= eps / 12) {
            data_.addIteration(0, 0, {x1, x2, x3}, {f1, f2, f3});
            ...
            data_.addIteration(seg.a, seg.b, {x_curr}, {f_x_curr});
        ...
        return data_.finishEval(seg.a, seg.b, x_curr);
    }
    ...
    EvalData brentsMethod(UndimFunction &f, Segment seg, double eps) {
        ...
        data_.addIteration(seg.a, seg.b, {x, w, v}, {fx, fw, fv});
        ...
        while (true) {
            ...
            u = (fabs(delta) >= fract1) ? x + delta : (delta > 0 ? x + fabs(fract1) :
                                                       x - fabs(fract1));
            fu = f(u);
            data_.addIteration(seg.a, seg.b, {u}, {fu});
            if (fu <= fx) {
                // got improvement
                if (u >= x) {
                    seg.a = x;
        ...
        return data_.finishEval(seg.a, seg.b, x);
    }
}
